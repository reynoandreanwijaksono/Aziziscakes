...
<nav x-data="{ mobileMenuOpen: false }" class="sticky top-0 z-50 backdrop-blur-md bg-white/90 border-b border-[#5a3825]/10 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
        <a href="{{ route('home') }}" class="playfair text-xl md:text-2xl text-[#5a3825] font-bold z-50"><i class="fas fa-cake-candles mr-2"></i>AZIZISCAKE</a>
        ...
        <div class="hidden md:flex items-center gap-4">
            ...
            <div class="relative ml-2" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-2" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-2" class="absolute right-0 mt-3 w-52 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50">
                    <a href="{{ route('orders.index') }}" class="block px-5 py-2.5 text-sm text-gray-700 hover:bg-[#fef3ec] hover:text-[#7a4b2b] transition-colors"><i class="fas fa-box mr-2"></i>Pesanan Saya</a>
                    @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.dashboard') }}" class="block px-5 py-2.5 text-sm text-gray-700 hover:bg-[#fef3ec] hover:text-[#7a4b2b] transition-colors"><i class="fas fa-gear mr-2"></i>Admin Panel</a>
                    @endif
                    <hr class="my-1.5 border-gray-100">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="w-full text-left px-5 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors"><i class="fas fa-sign-out-alt mr-2"></i>Keluar</button>
                    </form>
                </div>
            </div>
            @else
    ...
    <div x-show="mobileMenuOpen" x-transition x-cloak class="fixed inset-0 bg-white z-40 md:hidden pt-20 px-6 flex flex-col h-screen overflow-y-auto pb-24">
        ...
        <div class="mt-8 flex flex-col gap-4">
            @auth
            <a href="{{ route('orders.index') }}" class="w-full py-3 bg-[#fef9f5] text-[#7a4b2b] rounded-xl font-medium text-center" @click="mobileMenuOpen = false"><i class="fas fa-box mr-2"></i>Pesanan Saya</a>
            @if(auth()->user()->isAdmin())
            <a href="{{ route('admin.dashboard') }}" class="w-full py-3 bg-gray-100 text-gray-800 rounded-xl font-medium text-center" @click="mobileMenuOpen = false"><i class="fas fa-gear mr-2"></i>Admin Panel</a>
            @endif
            <form method="POST" action="{{ route('logout') }}" class="w-full mt-4">
                @csrf
                <button class="w-full py-3 border border-red-200 text-red-600 rounded-xl font-medium"><i class="fas fa-sign-out-alt mr-2"></i>Keluar</button>
            </form>
            @else
    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 mb-10">
            <div class="sm:col-span-2 md:col-span-1">
                <h3 class="playfair text-xl mb-4 text-[#f6f1eb]"><i class="fas fa-cake-candles mr-2"></i>Aziziscake</h3>
                <p class="text-white/70 text-sm leading-relaxed">{{ $footerText }}</p>
                <div class="flex gap-3 mt-5">
                    <a href="{{ $footerInstagram }}" class="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-[#7a4b2b] transition-colors text-base"><i class="fab fa-instagram"></i></a>
                    <a href="{{ $footerWhatsapp }}" class="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-[#7a4b2b] transition-colors text-base"><i class="fab fa-whatsapp"></i></a>
                    <a href="{{ $footerTiktok }}" class="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-[#7a4b2b] transition-colors text-base"><i class="fab fa-tiktok"></i></a>
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="mt-4 text-sm text-white/70">
                    <p><i class="fas fa-location-dot mr-2"></i>{{ $footerAddress }}</p>
                    <p class="mt-1"><i class="fas fa-phone mr-2"></i>{{ $footerPhone }}</p>
                </div>
            </div>
        </div>
        <div class="border-t border-white/15 pt-6 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-white/50 text-center md:text-left">
            ...
            <span>Dibuat dengan <i class="fas fa-heart text-red-500"></i> di Jepara, Jawa Tengah</span>
        </div>
    </div>
</footer>
...
<div class="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 flex justify-around items-center py-2 px-4 z-40 pb-[max(env(safe-area-inset-bottom),0.5rem)] shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
    <a href="{{ route('home') }}" class="flex flex-col items-center p-2 {{ request()->routeIs('home') ? 'text-[#7a4b2b]' : 'text-gray-400 hover:text-[#7a4b2b]' }}">
        <i class="fas fa-home text-xl leading-none mb-1"></i>
    ...
    <a href="{{ route('products.index') }}" class="flex flex-col items-center p-2 {{ request()->routeIs('products.*') ? 'text-[#7a4b2b]' : 'text-gray-400 hover:text-[#7a4b2b]' }}">
        <i class="fas fa-cake-candles text-xl leading-none mb-1"></i>
    ...
    <a href="{{ route('cart.index') }}" class="flex flex-col items-center p-2 relative {{ request()->routeIs('cart.*') ? 'text-[#7a4b2b]' : 'text-gray-400 hover:text-[#7a4b2b]' }}">
        <i class="fas fa-shopping-cart text-xl leading-none mb-1"></i>
    ...
    <a href="{{ route('orders.index') }}" class="flex flex-col items-center p-2 {{ request()->routeIs('orders.*') ? 'text-[#7a4b2b]' : 'text-gray-400 hover:text-[#7a4b2b]' }}">
        <i class="fas fa-box text-xl leading-none mb-1"></i>
    ...
    <a href="{{ route('login') }}" class="flex flex-col items-center p-2 text-gray-400 hover:text-[#7a4b2b]">
        <i class="fas fa-user text-xl leading-none mb-1"></i>
