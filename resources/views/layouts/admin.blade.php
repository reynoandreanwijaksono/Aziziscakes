    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-[#4b2e1e] text-white flex flex-col flex-shrink-0 transition-transform duration-300 md:relative md:translate-x-0"
        ...
        <div class="p-4 border-t border-white/10">
            ...
            <div class="flex gap-2">
                <a href="{{ route('home') }}" class="flex-1 text-center text-xs text-white/60 hover:text-white py-1.5 rounded-lg hover:bg-white/10 transition-colors"><i class="fa-solid fa-globe mr-1"></i> Website</a>
                <form method="POST" action="{{ route('logout') }}" class="flex-1">
                    @csrf
                    <button class="w-full text-xs text-red-300 hover:text-red-200 py-1.5 rounded-lg hover:bg-white/10 transition-colors"><i class="fa-solid fa-right-from-bracket mr-1"></i> Keluar</button>
                </form>
            </div>
        </div>
    </aside>
